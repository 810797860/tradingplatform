		...
		try {
			...
			try {
				...
				if (beanType.equals(HtmlBean)) {
					CtClass htmlBeanInterface = pool.get("com.secondhand.tradingplatformgeccoengine.spider.HtmlBean");
					clazz.addInterface(htmlBeanInterface);
				} else {
					CtClass jsonBeanInterface = pool.get("com.secondhand.tradingplatformgeccoengine.spider.JsonBean");
