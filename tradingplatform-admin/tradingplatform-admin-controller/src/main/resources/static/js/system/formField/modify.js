...
$(function(){
    var $save = $("#save"),
        $reset = $('#reset'),
        $form = $("#newField");
    if (!!$('#showType').attr('data-showtype')) {
        var showTypeSelect = JSON.parse($('#showType').attr('data-showtype')).id;
    } else {
        var showTypeSelect = null;
    }
    // 消息框位置控制
...
function initPage(){
    if (!!$('#formFieldId').val()) {
        ...
        //     var fieldtype = $('#fieldType').data('fieldtype').id;
        // } else {
        //     var fieldtype = null;
        // }
        // $('#fieldType').attr("data-fieldtype", fieldtype)
        var showType = $('#showType').data('showtype');
        if (!!showType) {
