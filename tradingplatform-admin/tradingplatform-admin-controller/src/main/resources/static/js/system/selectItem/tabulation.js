...
function responseHandler(res) {
    var length = res.data.recordsTotal;
    for (var i = 0; i < length; i++) {
        //处理模块分组
        var pidStr = res.data[i].pid;
        if(pidStr != undefined){
            var pidObj = JSON.parse(pidStr);
            res.data[i].pid = pidObj.title;
        }
    }
    return {
...
function queryParams(params) {
    ...
    if (StringNoEmpty(searchOptions.id)) postData['id'] = searchOptions.id;
    if (StringNoEmpty(searchOptions.title)) postData['title'] = searchOptions.title;
    if (StringNoEmpty(searchOptions.pid)) postData['pid'] = searchOptions.pid;
    return postData;
}
// 获取bootstrap table高度
function getHeight() {
