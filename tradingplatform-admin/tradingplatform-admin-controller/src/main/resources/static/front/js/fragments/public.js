...
NewAjax.prototype.sendRequest = function () {
    ...
    // 统一添加后缀
    if (typeof url === "string" && url.indexOf('?pc=true') < 0) {
        self.options.url += '?pc=true';
    }
    // 处理页面重定向
    if (typeof self.options.success === "function") {
        var stSave = self.options.success;
        self.options.success = function (res) {
            if (res.status === 403) {
                window.location.href = '/f/login.html?pc=true';
            } else if (res.status === 408) {
                pTipMessage('警告', '用户访问权限不足，请联系技术人员进行处理！', 'warning', 5000, true);
            } else {
                stSave(res);
            }
        }
...
function getLocalStorage(attrName) {
    ...
    var localSave = window.localStorage;
    return localSave.getItem(attrName);
}

// webSocket
function SOCKET(userId) {
    //嘉祥的
    // var headURl ='ws://10.0.20.31:8188?';
    //宇昂的
    ...
    //线上生产环境
    // var headURl ='ws://120.77.200.203:8188?';
    // 存储this
    var _this = this;
    // 定义socket
    var socket = null;
    ...
    function socketReconnect() {
        // 清除旧socket 数据与方法
        socket = null;
        console.log('准备重启', socketUrl);
        // 重新连接
        socket = _this.init(socketUrl);
        console.log('重启完毕，正在链接');
        // 初始化socket监听
        initSocketEvent();
    }
...
function readTextFile() {
    $.ajax({
        url: '/static/front/assets/json/sensitiveWords.json',
        async: false,
        dataType: 'json',
        scriptCharset: 'UTF-8',
        success: function (res) {
