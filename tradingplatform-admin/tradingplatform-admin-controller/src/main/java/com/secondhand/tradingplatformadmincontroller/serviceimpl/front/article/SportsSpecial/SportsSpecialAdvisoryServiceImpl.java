...
public class SportsSpecialAdvisoryServiceImpl extends BaseServiceImpl<SportsSpecialAdvisoryMapper, SportsSpecialAdvisory> implements SportsSpecialAdvisoryService {
    ...
    public Page<Map<String, Object>> mySelectPageWithParam(Page page, SportsSpecialAdvisory sportsSpecialAdvisory) {
        ...
        //自动生成sql回显
        wrapper.setSqlSelect("c_business_sports_special_advisory.id as id, ( SELECT concat( '{\"id\":\"', sbu.id, '\",\"user_name\":\"', sbu.user_name, '\",\"phone\":\"', IFNULL(sbu.phone, ''), '\"}' ) FROM s_base_user sbu WHERE (sbu.id = c_business_sports_special_advisory.reply_id) ) AS reply_id, c_business_sports_special_advisory.created_by as created_by, ( SELECT concat( '{\"id\":\"', cbss.id, '\",\"title\":\"', cbss.title, '\",\"cover\":\"', cbss.cover, '\",\"price\":\"', cbss.price, '\"}' ) FROM c_business_sports_special cbss WHERE (cbss.id = c_business_sports_special_advisory.sports_id) ) AS sports_id, c_business_sports_special_advisory.deleted as deleted, c_business_sports_special_advisory.content as content, c_business_sports_special_advisory.description as description, c_business_sports_special_advisory.updated_at as updated_at, (select concat('{\"id\":\"', sbsi.id, '\",\"pid\":\"', sbsi.pid, '\",\"title\":\"', sbsi.title, '\"}') from s_base_select_item sbsi where (sbsi.id = c_business_sports_special_advisory.back_check_status)) AS back_check_status, c_business_sports_special_advisory.pid as pid, c_business_sports_special_advisory.updated_by as updated_by, c_business_sports_special_advisory.not_pass_reason as not_pass_reason, ( SELECT concat( '{\"id\":\"', sbu.id, '\",\"user_name\":\"', sbu.user_name, '\",\"phone\":\"', IFNULL(sbu.phone, ''), '\"}' ) FROM s_base_user sbu WHERE (sbu.id = c_business_sports_special_advisory.user_id) ) AS user_id, c_business_sports_special_advisory.created_at as created_at");
        //遍历排序
