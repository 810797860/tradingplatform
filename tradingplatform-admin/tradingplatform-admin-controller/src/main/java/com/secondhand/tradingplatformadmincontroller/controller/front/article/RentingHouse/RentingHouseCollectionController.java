...
public class RentingHouseCollectionController extends BaseController {
    ...
    /**
     * @description : 用户获取收藏rentingHouse的ids列表
     * @author : zhangjk
     * @since : Create in 2019-03-27
     */
    @GetMapping(value = "/query_collection", produces = {MediaType.APPLICATION_JSON_VALUE})
    @ApiOperation(value = "/query_collection", notes="用户获取收藏rentingHouse的ids列表")
    @ResponseBody
    public JsonResult<List<Object>> getRentingHouseAdvisoryCollectionList() {
        Subject subject = SecurityUtils.getSubject();
        JsonResult<List<Object>> resJson = new JsonResult<>();
        try {
            //检查是否具有权限
            subject.checkPermission("/front/rentingHouseCollection/query_collection");
            Session session = subject.getSession();
            Long userId = Long.valueOf(session.getAttribute(MagicalValue.USER_SESSION_ID).toString());
            resJson.setData(rentingHouseCollectionService.mySelectCollectionList(userId));
