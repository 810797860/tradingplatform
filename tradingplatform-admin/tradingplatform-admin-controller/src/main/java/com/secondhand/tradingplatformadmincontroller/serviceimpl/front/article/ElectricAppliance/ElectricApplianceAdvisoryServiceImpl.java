...
public class ElectricApplianceAdvisoryServiceImpl extends BaseServiceImpl<ElectricApplianceAdvisoryMapper, ElectricApplianceAdvisory> implements ElectricApplianceAdvisoryService {
    ...
    public Page<Map<String, Object>> mySelectPageWithParam(Page page, ElectricApplianceAdvisory electricApplianceAdvisory) {
        ...
        //自定义生成sql回显
        wrapper.setSqlSelect("c_business_electric_appliance_advisory.id as id, c_business_electric_appliance_advisory.updated_at as updated_at, (select concat('{\"id\":\"', sbsi.id, '\",\"pid\":\"', sbsi.pid, '\",\"title\":\"', sbsi.title, '\"}') from s_base_select_item sbsi where (sbsi.id = c_business_electric_appliance_advisory.back_check_status)) AS back_check_status, c_business_electric_appliance_advisory.pid as pid, c_business_electric_appliance_advisory.updated_by as updated_by, c_business_electric_appliance_advisory.not_pass_reason as not_pass_reason, ( SELECT concat( '{\"id\":\"', sbu.id, '\",\"user_name\":\"', sbu.user_name, '\",\"phone\":\"', IFNULL(sbu.phone, ''), '\"}' ) FROM s_base_user sbu WHERE (sbu.id = c_business_electric_appliance_advisory.user_id) ) AS user_id, ( SELECT concat( '{\"id\":\"', sbu.id, '\",\"user_name\":\"', sbu.user_name, '\",\"phone\":\"', IFNULL(sbu.phone, ''), '\"}' ) FROM s_base_user sbu WHERE (sbu.id = c_business_electric_appliance_advisory.reply_id) ) AS reply_id, c_business_electric_appliance_advisory.created_by as created_by, ( SELECT concat( '{\"id\":\"', cbea.id, '\",\"title\":\"', cbea.title, '\",\"cover\":\"', cbea.cover, '\",\"price\":\"', cbea.price, '\"}' ) FROM c_business_electric_appliance cbea WHERE (cbea.id = c_business_electric_appliance_advisory.electric_id) ) AS electric_id, c_business_electric_appliance_advisory.deleted as deleted, c_business_electric_appliance_advisory.content as content, c_business_electric_appliance_advisory.description as description, c_business_electric_appliance_advisory.created_at as created_at");
        //遍历排序
