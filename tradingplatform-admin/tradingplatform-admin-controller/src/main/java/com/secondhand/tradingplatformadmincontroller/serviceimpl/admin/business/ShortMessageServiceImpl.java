...
public class ShortMessageServiceImpl implements ShortMessageService {
    ...
    public SendSmsResponse sendVerificationCode(HttpServletRequest httpServletRequest, String phoneNumbers) throws ClientException {
        ...
        session.setAttribute(MagicalValue.STRING_OF_VERIFICATION, verificationCode);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public SendSmsResponse notifyPurchaseSuccess(String phoneNumbers) throws ClientException {
        ...
        request.setSignName("校园二手交易市场网");
        //必填:短信模板-可在短信控制台中找到，发送国际/港澳台消息时，请使用国际/港澳台短信模版
        request.setTemplateCode("SMS_166095769");
        //可选:模板中的变量替换JSON串,如模板内容为"亲爱的${name},您的验证码为${code}"时,此处的值为
        //友情提示:如果JSON中需要带换行符,请参照标准的JSON协议对换行符的要求,比如短信内容中包含\r\n的情况在JSON中需要表示成\\r\\n,否则会导致JSON在服务端解析失败
//        request.setTemplateParam("{\"name\":\"" + name + "\"}");
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public SendSmsResponse dailyTextMessage(String city, String weather, String tem1, String tem2, String clothes, String yi, String ji, String phoneNumbers) throws ClientException {
        ...
        //必填:短信模板-可在短信控制台中找到，发送国际/港澳台消息时，请使用国际/港澳台短信模版
        request.setTemplateCode("SMS_166655135");
        //可选:模板中的变量替换JSON串,如模板内容为"亲爱的${name},您的验证码为${code}"时,此处的值为
        //友情提示:如果JSON中需要带换行符,请参照标准的JSON协议对换行符的要求,比如短信内容中包含\r\n的情况在JSON中需要表示成\\r\\n,否则会导致JSON在服务端解析失败
        request.setTemplateParam("{\"city\":\"" + city + "\", " +
                "\"weather\":\"" + weather + "\", " +
                "\"tem1\":\"" + tem1 + "\", " +
                "\"tem2\":\"" + tem2 + "\", " +
                "\"clothes\":\"" + clothes + "\", " +
